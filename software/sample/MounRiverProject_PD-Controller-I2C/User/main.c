...
int main(void)
{
    ...
    for( u8 j = 0 ; j < 0x7F ; j ++ )
    {
        n = I2C_read(0b0100010,j,20,buf); // PD address ; kono address dato TRANSMITTERMODE ni naranai
        //n = I2C_read(0b1110100,0,20,buf); //LED address; kono address dato RECEIVERMODE ni naranai

        printf("%4x, %d byte : ",j,n);
        for ( u8 i = 0; i < n ; i ++)
        {
            printf("%d, ",buf[i]);
        }
